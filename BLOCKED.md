...
14. 帧质量与 rendered：FIX-BATCH-015 已 PR CI verified（PR #41 / CI `33447491953`），能区分 expected/loaded/failed、完整/partial/全失败，但质量状态尚未通过创建 API 写入 ComparisonReceipt；也没有浏览器画布像素/用户确认回执。因此 OMB-AUD-014 仅 partial，OMB-AUD-015 仍 open，任何合成四屏或服务器 200 都不是 `rendered/accepted`。
