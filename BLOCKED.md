...
9. 本机容量：2026-08-28 17:12 `/System/Volumes/Data` 约 5.8 GiB、17:22 最新约 7.2 GiB，仍低于 8 GiB 执行门。本机测试、构建、浏览器、下载、截图和影像生成暂停；不得通过删除用户数据规避；第一批已由隔离 GitHub CI 验证。
10. 全量审计：`docs/问题账本.md` 记录 38 组问题。`FIX-BATCH-001` 已由 main `5a7e9ad`/CI `33159198541` 处理 `OMB-AUD-001/003/005/023` 和 `002` 的假绿子项；其余不得因第一批合并而标记解决。
