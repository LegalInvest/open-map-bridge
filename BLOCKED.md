...
9. 本机容量：2026-08-28 19:09 `/System/Volumes/Data` 约 5.2 GiB，低于 8 GiB 执行门。本机测试、构建、浏览器、下载、截图和影像生成暂停；不得通过删除用户数据规避；后续候选由隔离 GitHub CI 验证。
10. 全量审计：`docs/问题账本.md` 记录 38 组问题。`FIX-BATCH-001/002/003/004/005/006` 已进入 main；001/003/004/005/010/020/021/022/023 达到 main，002 仅假绿和解码前置子项达到 main。FIX-BATCH-007/OMB-AUD-018 只是 `local-candidate`；其余不得因候选或六批合并而标记解决。
...
12. 导入资源边界：预览 LRU 和 QR 图片字节/像素/倍率门已形成候选；PR #13 首次 CI `33166210759` 虽通过单测，但 Web 测试 fixture 未过 BlobPart 严格类型门，构建/Chrome 未运行。全绿并合并前不能声称非可信图片资源耗尽路径已关闭。该批不解决 confirm 消费时机、存储错误分类或二维码相机视频帧资源治理。
