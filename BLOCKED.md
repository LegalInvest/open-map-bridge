...
12. 三端部署：runtime source/Tencent current=`7da03c3`，runtime main CI=`33435153343`；部署证据 docs-only main=`84f103f`，main CI=`33436199094`。gateway/Web hash、项目 Node、systemd、宝塔 nginx include、双回环、health/vault 和持久 state/vault hash 均通过，保留 `ccd3cd8`/`16e805d` 等回退制品；docs-only 后代不重复发布。重启窗口首次 nginx health 短暂 502 后恢复。真实 Ovi 与真实通用 source 业务验收均独立阻塞。
