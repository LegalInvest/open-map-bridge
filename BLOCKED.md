...
9. 本机容量：2026-08-31 15:32 根卷约 10 GiB，高于 8 GiB 执行门，可运行有界测试和构建；低于 8 GiB 时立即停止浏览器、构建、下载和影像生成，不通过删除用户数据规避。
10. 全量审计：`docs/问题账本.md` 当前记录 39 组问题。FIX-BATCH-001–008 已进入 main，11 组达到 main、28 组仍未闭合；FIX-BATCH-009 的 OMB-AUD-006/027 仅为 local-verified，其余问题不得因生产冒烟通过而标记解决。
