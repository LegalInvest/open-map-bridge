...
9. 本机容量：2026-08-28 17:43 `/System/Volumes/Data` 约 6.1 GiB，低于 8 GiB 执行门。本机测试、构建、浏览器、下载、截图和影像生成暂停；不得通过删除用户数据规避；候选由隔离 GitHub CI 验证。
...
11. 真实 ready 晋级：configured 隐藏、空 probe 失败和图片解码/内容验证已进入 main，但本地凭证保险库、请求时 DNS/IP/重绑定门、最小真实瓦片请求、探测证据/ready 编排和真实日期目录仍缺，`OMB-AUD-002/004/020` 保持开放；`OMB-AUD-021` 已达到 main，但不等于真实源验收。
