...
12. 三端部署：运行源码 `ccd3cd8`、部署证据 main `601ac68`，GitHub main CI `33430487066` 全绿；腾讯 current 为 release `ccd3cd8`，gateway/Web hash、项目 Node、systemd、宝塔 nginx include、双回环、health/vault 和持久 state hash 均通过，保留 `16e805d` 等回退制品。014C 仍仅在功能分支，不能描述为 main/deployed；真实 Ovi 与真实通用 source 业务验收均独立阻塞。
