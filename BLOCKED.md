...
9. 本机容量：2026-08-31 15:22 根卷约 11 GiB，高于 8 GiB 执行门，可运行有界测试和构建；低于 8 GiB 时立即停止浏览器、构建、下载和影像生成，不通过删除用户数据规避。
10. 全量审计：`docs/问题账本.md` 当前记录 39 组问题。`FIX-BATCH-001/002/003/004/005/006/007` 已进入 main，10 组达到 main；`FIX-BATCH-008 / OMB-AUD-039` 已本地验证但尚未进入 GitHub main，其余问题不得因当前自动门通过而标记解决。
...
12. 三端部署：用户已把服务器纳入最终验收，但项目内没有权威服务器、域名、数据目录、回滚点和秘密注入契约。当前只发现本机 SSH 别名，不能据此猜测部署目标；必须先形成可复现生产制品，再对候选主机做只读盘点。
