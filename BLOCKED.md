...
9. 本机容量：2026-09-01 05:56 根卷可用 `7,562,484 KiB`（约 7.21 GiB），低于 8 GiB 硬门；FIX-BATCH-015 在跌破前已完成 3 Node＋260 Vitest、8 typecheck、build/smoke、4 Chrome。现已停止新测试、构建、浏览器、下载、影像和部署；只读检查未发现本项目残留进程，swap 使用约 5.37 GiB。不得通过删除用户数据、项目证据或真实运行数据规避。
...
12. 三端部署：本地/GitHub main=`c6cd5be`，runtime source/Tencent current=`d350ac3`，runtime main CI=`33439685686`，最新 docs main CI=`33441198030`。FIX-BATCH-015 仍只在本地候选分支，腾讯尚未含帧质量修复。gateway/Web hash、项目 Node、systemd、宝塔 nginx include、双回环、health/vault 和持久 state/vault hash 均通过；真实 Ovi 与真实通用 source 业务验收仍独立阻塞。
14. 帧质量与 rendered：FIX-BATCH-015 已 local-verified，能区分 expected/loaded/failed、完整/partial/全失败，但质量状态尚未通过创建 API 写入 ComparisonReceipt；也没有浏览器画布像素/用户确认回执。因此 OMB-AUD-014 仅 partial，OMB-AUD-015 仍 open，任何合成四屏或服务器 200 都不是 `rendered/accepted`。
