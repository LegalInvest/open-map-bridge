...
9. 本机容量：2026-08-28 18:40 `/System/Volumes/Data` 约 5.3 GiB，低于 8 GiB 执行门。本机测试、构建、浏览器、下载、截图和影像生成暂停；不得通过删除用户数据规避；后续候选由隔离 GitHub CI 验证。
10. 全量审计：`docs/问题账本.md` 记录 38 组问题。`FIX-BATCH-001/002/003/004` 已进入 main；001/003/004/005/020/021/023 达到 main，002 仅假绿和解码前置子项达到 main；FIX-BATCH-005/OMB-AUD-022 只是 `local-candidate`，其余不得因候选或四批合并而标记解决。
...
12. 时序输入真值：共享严格 schema 已形成候选，但在 PR CI 通过前，不能声称旧 API、V1、SDK 和适配器的日期/ID/坐标行为已一致；该候选不解除真实源阻塞。
