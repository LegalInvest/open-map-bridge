...
- `FIX-BATCH-005` 已形成 `local-candidate`：`@omb/temporal-source` 统一实际 ISO 日期、顺序窗口、受限 AOI/date ID、规范十进制安全整数、z≤30 与 x/y<2^z；旧时序 API、V1、SDK、Ovi/合成适配器复用。新增伪日期、反向窗口、空白/超长 ID、前导零/科学计数/小数和越界瓦片反例；待 PR CI，零真实外联。
...
| Git 仓库 | GitHub main `1e7308f`；当前候选分支 `codex/audit-p1-temporal-input-truth` 基于该提交，remote 为 `https://github.com/LegalInvest/open-map-bridge.git` | `git branch --show-current`、`git rev-parse HEAD origin/main`、`git remote -v` | 2026-08-28 18:40 |
| 根卷空间 | 18:40 约 5.3 GiB，低于 8 GiB 门；停止本机构建、测试、浏览器、下载和缓存 | `df -h /System/Volumes/Data` | 2026-08-28 18:40 |
...
| 审计问题账本 | 161 个 tracked files 静态审计形成 38 组问题与公开 backlog；前四批已进 main，第五批统一时序输入真值并待 CI | FIX-BATCH-001/002/003/004 main / FIX-BATCH-005 local-candidate / remaining discovered | `docs/问题账本.md`、PR #1/#3/#5/#7、既有 CI；2026-08-28 18:40 |
| 工作区基线 | GitHub main `1e7308f`；当前 `codex/audit-p1-temporal-input-truth` 含 FIX-BATCH-005 产品/文档候选；约 5.3 GiB 低于 8 GiB 门 | main verified / candidate unverified / capacity blocked | `git status/log/rev-parse`、`df`；2026-08-28 18:40 |
...
| OMB-AUD-022 / IF-006/007 | 时序输入真值 | FIX-BATCH-005 候选把实际日期、窗口顺序、ID 长度/控制符、规范路径整数和 Web Mercator 瓦片边界下沉为共享 schema；旧 API、V1、SDK 和适配器使用同一规则 | `packages/temporal-source/src/schema.ts`、`apps/gateway/src/routes/temporal-input.ts`、SDK client | package schema、旧 API、V1、SDK 正反例已写；待 PR CI | local-candidate | 本机容量门下未执行；CI 前不晋级 main | 2026-08-28 18:40 |
