...
- `FIX-BATCH-007` 已形成 `local-candidate`：预览 store 主动 TTL 清理并以访问顺序实施 64 条/4 MiB LRU；QR 图片在 object URL/ZXing 前限制声明/实际 8 MiB，解析 PNG/JPEG/WebP 头并限制 16,777,216 像素，浏览器尺寸须一致，2×/3× 受缩放像素预算约束。PR #13 首次 CI `33166210759` 单测通过后在测试 fixture 的 BlobPart 类型门失败；已改明确 ArrayBuffer，待重跑。
...
| OMB-AUD-018 / NFR-004/006 | 导入非可信输入资源门 | FIX-BATCH-007 候选为 preview store 增加 TTL/64 条/4 MiB LRU，为 QR 图片增加解码前 8 MiB/16 MiPx/格式头/倍率门 | preview store、`image-dimensions.ts`、QR reader | 首次 CI `33166210759` 单测通过、Web typecheck fixture 失败；明确 ArrayBuffer 修正待重跑 | local-candidate | 构建/Chrome 未运行；相机视频帧与 OMB-AUD-019 仍开放 | 2026-08-28 19:12 |
