...
- 2026-09-01 02:48 容量恢复到 `10,332,424 KiB` 后启动 014B。`MapSourceDefinition` 新增 transport scheme 与逐字段 provenance，旧持久记录兼容加载但标记 `legacy-unknown`；QR/`.ovmap` 只保留瓦片变量或保守白名单内的非秘密常量 query，其余固定值继续脱敏；OMS 归一化不再丢失已声明的 protocol/projection/zoom/tile/format/hosts/attribution/license。通用静态策略对未知、未确认、旧版和 HTTP 请求计划 fail closed，准备度指纹纳入完整公开请求计划；252 Vitest＋3 Node、8 workspace typecheck、production build/smoke 与交底新鲜度本地通过，零真实 DNS/HTTP/二维码载荷，阶段仅 `local-verified / not main`。
