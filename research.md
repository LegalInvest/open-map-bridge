...
- 2026-08-31 用户授权自主推进到本机、GitHub、服务器最新可验收。当前 GitHub main/origin 为 `9b73949`，CI `33371357929` 全绿；腾讯 current `62ab114` 服务 active、health=`atomic-json`、4174/8080 双回环监听。39 组问题中 15 组达到 main、24 组仍未闭合，真实源验收继续独立开放。`FIX-BATCH-011` 的 Ovi 最小 probe/ready 编排已本地验证但尚未进入 main。
...
- `FIX-BATCH-011` 当前为 `local-verified`：严格解析最多 500 条非秘密日期元数据和单个 probe 坐标；probe date 必须来自可请求目录项。应用启动时用既有 Ovi 回环路径请求一个瓦片，只有 200、非空、合法 PNG/JPEG 且完整解码/尺寸门通过才赋予 runtime ready 和 V1 `temporal-catalog/tiles`，403、损坏图片或未配置 probe 均保持 configured。182 个 Vitest＋2 个 Node 测试和 8 workspace typecheck 通过；无用户真实源请求，未持久化 ProbeResult，非 main/deployed/accepted。
...
| Git 仓库 | GitHub main/origin `9b73949`；当前分支 `codex/fix-batch-011-ovi-probe-ready` 基于该提交，remote 为 `https://github.com/LegalInvest/open-map-bridge.git` | `git branch --show-current`、`git rev-parse HEAD origin/main`、CI `33371357929` | 2026-08-31 17:24 |
| 根卷空间 | 约 9.0 GiB，刚超过 8 GiB 门；测试/类型检查已通过，build/production/E2E 优先交 GitHub CI | `df -Pk /System/Volumes/Data` | 2026-08-31 17:37 |
...
| Ovi Web 桥接 | 官方文档证明接口形态；main 只接受回环 origin/注入日期且已删除虚构年度目录。FIX-BATCH-011 候选用一个已登记瓦片通过完整图片门后才 runtime ready；本机官方服务尚未启用和请求 | date/response truth main；probe/ready local-verified candidate；real compatibility blocked | 182 Vitest＋2 Node＋8 typecheck；需验证官方监听、持久 ProbeResult、真实目录 provider 和特殊历史源出图 |
...
| 二次开发 V1 | 脱敏源目录、能力协商、严格应用清单、TypeScript SDK 和本地日期/瓦片消费已实现；configured OviBridge 不授予运行能力；FIX-BATCH-011 候选仅在受控 probe 解码成功后赋予 ready 能力 | main for existing V1 / probe-ready local-verified candidate | `packages/developer-sdk`、`apps/gateway/src/{app,developer,routes/developer}.ts`、定向 route 测试 |
...
| 审计问题账本 | 39 组问题与公开 backlog；15 组达到 main、24 组未闭合。FIX-BATCH-011 正推进 OMB-AUD-002 的 probe/ready 子项 | FIX-BATCH-001–010 main/deployed as recorded；011 local-verified | `docs/问题账本.md`、main CI `33371357929`、当前定向门；2026-08-31 17:24 |
| 工作区基线 | GitHub main/origin `9b73949`；FIX-BATCH-011 分支含未提交候选；约 9.0 GiB，刚超过容量门 | product main / local-verified candidate / capacity low-margin | `git status/log/rev-parse`、`df -Pk`；2026-08-31 17:37 |
| 自动测试 | 当前 main 的远端 CI `33371357929` 全绿；FIX-BATCH-011 本地 182 Vitest＋2 Node＋8 workspace typecheck 通过，完整 build/production/E2E 待 PR CI | main CI verified / candidate local-verified | 本轮命令输出；2026-08-31 17:37 |
| GitHub main | 公共唯一主仓 `LegalInvest/open-map-bridge`；main/origin `9b73949`，当前候选尚未创建 PR | main through FIX-BATCH-010 evidence；FIX-BATCH-011 local only | CI `33371357929`；2026-08-31 17:24 |
...
| OMB-AUD-021 / NFR-004/006 | Ovi 响应资源与图片真实性 | main `de36012` 使用 Web Stream 逐块限制 5 MiB，只接受 200，其他状态丢弃正文，成功仅接受 PNG/JPEG 并检查 magic、完整解码、单边 2048 和 RGBA 长度；FIX-BATCH-011 候选复用同一门决定 runtime ready | `apps/gateway/src/temporal/{ovi-bridge,image-validation}.ts` | main 反例由 CI `33160315934` 通过；候选 probe 成功/403/损坏图由定向测试通过 | response gate main / ready orchestration local-verified | ProbeResult 持久化、真实源和通用请求时 DNS/IP 门仍缺 | 2026-08-31 17:24 |
...
| JRN-007 / FR-009 / OMB-AUD-004 | 历史源日期和真实瓦片 | 合成 20 年源已验证；configured Ovi 从消费端隐藏；main 不伪造日期。FIX-BATCH-011 候选仅以严格目录中的单个请求执行回环 probe，完整图片门通过才 ready | `packages/temporal-source`、`apps/gateway/src/{app,config,temporal,routes/temporal}.ts` | 目录真值 CI `33163589956`；候选 182 Vitest＋2 Node＋8 typecheck；AC-011 未运行 | date truth main / probe-ready local-verified / real adapter blocked | 缺真实日期目录 provider、持久 ProbeResult、四日期互异和用户真实源 | 2026-08-31 17:37 |
...
| JRN-011 / FR-015/016 / IF-007 | 基于导入图源二次开发 | opaque Ovi runtime 与 imported UUID 合并；configured 仅 metadata。FIX-BATCH-011 候选在同 UUID 探针解码成功后才开放 temporal-catalog/tiles | `packages/developer-sdk`、`apps/gateway/src/{app,config,developer,routes/developer}.ts` | 同 UUID/configured main 回归；候选 ready descriptor/date route 定向测试通过 | binding main / probe-ready local-verified | 仍缺 vault、持久 ProbeResult、真实源；SDK 尚无可发布制品 | 2026-08-31 17:24 |
