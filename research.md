...
- 2026-09-01 02:48 容量恢复到 `10,332,424 KiB` 后启动 014B。`MapSourceDefinition` 新增 transport scheme 与逐字段 provenance，旧持久记录兼容加载但标记 `legacy-unknown`；QR/`.ovmap` 只保留瓦片变量或保守白名单内的非秘密常量 query，其余固定值继续脱敏；OMS 归一化不再丢失已声明的 protocol/projection/zoom/tile/format/hosts/attribution/license。通用静态策略对未知、未确认、旧版和 HTTP 请求计划 fail closed，准备度指纹纳入完整公开请求计划；252 Vitest＋3 Node、8 workspace typecheck、production build/smoke、交底新鲜度与 4 条公开 E2E 本地通过，零真实 DNS/HTTP/二维码载荷。PR #32 首轮 CI `33428136462` 在公开 QR E2E 保留红灯：相对 `ul` 的未知 scheme 使任务停在 network-policy，而旧断言期待 runtime-binding；改为短显式 HTTPS 夹具后完整公开旅程本地 4/4 通过，待远端复验。

### 本轮回答

- 固化产品旅程、规则、数据、接口、安全和验收。
- 记录当前 `.ovmap`/二维码证据、开源候选、项目资产和技术推荐。
- 当前主线是 `QR/.ovmap → confirmed → vault/DNS/probe → ready → 真实渲染 → 四期/SDK`；不抓取大范围地图、不部署公网、不连接未经批准的企业图源，也不执行任意第三方插件代码。腾讯私有 loopback 应用部署已获单独授权并完成技术验证。
...
| Git 仓库 | 本地/GitHub main `32cb36d`，PR/main CI `33406940553`/`33407144250` 全绿；腾讯 current `3bbcbfa`；当前为 FIX-BATCH-013 docs-only 证据分支 | `git branch --show-current`、`git rev-parse HEAD origin/main`、remote `ls-remote`、CI、腾讯 readlink | 2026-08-31 23:15 |
| 根卷空间 | 约 8.08 GiB、swap 使用约 8.15 GiB，虽刚过门但无安全余量；不执行新的本地重型门，只进行小型状态更新 | `df -Pk /System/Volumes/Data`、`sysctl vm.swapusage` | 2026-08-31 23:15 |
...
上述导入路径中的开放 schema、显式状态机、QR 图片/摄像头、`ovobj` 两种真实结构、`.ovmap record37-zlib`、零上游预览、授权确认、原子 JSON 保存和 confirmed 回执已进入既有 main；本地加密凭证保险库已部署。请求时 DNS/IP/连接固定/重绑定传输代码已 main，但尚未接入 probe/tile；通用瓦片代理和真实渲染仍缺。时序路径有合成日期/AOI/四期/卷帘/播放 UI，但审计确认 AOI 拓扑、部分瓦片、时间轴帧真值和观察持久化仍有问题。已有公共 CI 与私有 loopback 部署，不存在公网部署或用户独立签收。
...
| 二次开发 V1 | 脱敏源目录、能力协商、严格应用清单、TypeScript SDK 和本地日期/瓦片消费已实现；configured OviBridge 不授予运行能力；FIX-BATCH-011 仅在受控 probe 解码成功后赋予 ready 能力 | code deployed / real source blocked | `packages/developer-sdk`、`apps/gateway/src/{app,developer,routes/developer}.ts`、PR/main CI、本地 production smoke、腾讯 current `3bbcbfa` |
...
| 审计问题账本 | 40 组问题与公开 backlog；FIX-BATCH-012 vault 已 deployed；FIX-BATCH-013 的 OMB-AUD-002 DNS 传输子项已 main，整体仍 partial | FIX-BATCH-001–013 as recorded | `docs/问题账本.md`、PR/main CI、本地 targeted tests、腾讯 release evidence；2026-08-31 23:15 |
| 工作区基线 | main `32cb36d` / current `3bbcbfa`；本机约 8.08 GiB | vault deployed / DNS transport main-not-wired | `git status/log/rev-parse/ls-remote`、`df -Pk`；2026-08-31 23:15 |
| 自动测试 | PR #25 CI `33406940553` 与 main CI `33407144250` 全绿，包含 disclosure/unit/typecheck/build/production smoke/公开 Chrome；PR #23 前两轮历史红灯继续保留 | FIX-BATCH-013 exact main CI verified | GitHub CI；2026-08-31 23:14 |
| GitHub main | 公共唯一主仓 `LegalInvest/open-map-bridge`；main `32cb36d` 含 FIX-BATCH-013；腾讯 current `3bbcbfa` 含已部署 vault | DNS transport main-not-wired / vault deployed | PR #25、CI `33406940553`/`33407144250`；2026-08-31 23:14 |
| 部署 | 腾讯私有 loopback current `3bbcbfa`；retained `94e42b1`、`62ab114` 与 rollback `33f7f06` | vault deployed / synthetic browser journey prior verified | gateway/Web hash、health/vault、401、双回环和状态 hash 前后不变；真实 Ovi 源未验收 |
...
|---|---|---|---|---|---|---|---|
| JRN-001 / FR-001 | 真实二维码到出图 | 用户 QR 已在 Chrome 解码成脱敏预览；合成 QR 已授权保存/刷新；标准 query/header 凭证可进入已部署本地加密 vault | `apps/web/src/import`、`packages/qr-import`、gateway credential vault | AC-001 partial | preview main / vault deployed / render blocked | Ovi 私有不透明值仍仅走官方桥；未写真实凭证，未探测/出图 | 2026-08-31 23:15 |
...
| FR-004 / BR-004/005 | 安全预览、零外联、秘密 | 检查零上游、秘密剥离、授权前后端门已实现；FIX-BATCH-012 vault deployed；FIX-BATCH-013 DNS/IP 固定传输 main | gateway import/security、Web import | unit/route/E2E | vault deployed / DNS transport main / overall partial | 固定传输未接 probe/tile | 2026-08-31 23:15 |
...
| NFR-001 | SSRF/解压/开放代理防护 | 静态策略已 main；FIX-BATCH-013 增加请求时全 DNS 答案检查、metadata 永久阻断、精确私网例外、单 IP lookup pin、peer 复核和禁自动重定向/authority 头覆盖 | `apps/gateway/src/security/{source-policy,upstream-network}.ts` | 37 专项 tests；PR/main CI `33406940553`/`33407144250` 全门 | static/request transport main | 尚未接入 source probe/tile，不 deployed | 2026-08-31 23:15 |
| OMB-AUD-021 / NFR-004/006 | Ovi 响应资源与图片真实性 | main `de36012` 使用 Web Stream 逐块限制 5 MiB，只接受 200，其他状态丢弃正文，成功仅接受 PNG/JPEG 并检查 magic、完整解码、单边 2048 和 RGBA 长度；FIX-BATCH-011 复用同一门决定 runtime ready | `apps/gateway/src/temporal/{ovi-bridge,image-validation}.ts` | 响应反例 CI `33160315934`；probe 成功/403/损坏图由既有 CI 全门通过；腾讯 current `3bbcbfa` 保留该运行能力 | code deployed | ProbeResult 持久化、真实源和通用安全传输接线仍缺 | 2026-08-31 23:15 |
| OMB-AUD-020 / IF-006 / NFR-001/004 | 本地 gateway 入站信任边界 | main `1d0ebc4` 校验精确 Host/Origin/Fetch-Site、Bearer、app 权限、CSRF 和限流；FIX-BATCH-012 vault 已部署；FIX-BATCH-013 上游固定传输已 main | gateway access/config/Vite/SDK + credential vault + upstream network | existing CI + PR #23/#25/main CI | inbound/DNS transport main / vault deployed | 上游安全传输尚未接 probe/tile | 2026-08-31 23:15 |
...
| AC-001 至 AC-010 | 业务验收 | AC-002/003 本地通过；AC-001 完成真实预览但未出图；vault 配置候选仅证明加密存储 | E2E + 真实浏览器 | 对应 AC | mixed, no cross-level rollup | 下一门为 vault main/deploy、DNS/IP/连接固定、凭证注入、probe/render | 2026-08-31 18:51 |
...
| JRN-011 / FR-015/016 / IF-007 | 基于导入图源二次开发 | opaque Ovi runtime 与 imported UUID 合并；configured 仅 metadata。FIX-BATCH-011 在同 UUID 探针解码成功后才开放能力；FIX-BATCH-012 vault 已部署；FIX-BATCH-013 固定传输已 main | SDK、gateway app/config/developer/import/security | existing main CI + 37 DNS transport assertions | binding/probe-ready/vault deployed / DNS main-not-wired | DNS 传输未接主链，仍缺持久 ProbeResult、真实源，SDK 尚无可发布制品 | 2026-08-31 23:15 |
| JRN-012 / UI-008 / FR-017 | 可观察一键任务 | confirmed source 可创建四步零外联准备度 run；FIX-BATCH-012 把凭证步骤从“非空引用”提升为 active vault 实际命中 | automation schema/service/routes/Web | existing main CI + PR #23/main CI | base/vault main / AC-019/021 partial | 无 DNS/HTTP 探测、步骤级续跑/取消、AOI/四期步骤；不能标 ready | 2026-08-31 19:30 |
...
2. **二维码私有协议**：官方未公开 wire schema。用户样本证明至少存在普通模板与 `at/ad/al + opaque ul` 两种变体；未知字段继续失败关闭。普通 query/header 值可以进入通用 vault，奥维不透明字段只能走官方桥或有合法格式证据的专用适配器，均不得进入普通 JSON。
...
| 运行链 | Web UI 到真实出图 | 双入口/合成消费链有既有 CI；FIX-BATCH-001/002/011 修复同 UUID/configured/probe 图片真值；FIX-BATCH-012 vault 已部署；FIX-BATCH-013 传输已 main | deployed vault + main-not-wired DNS transport + issue ledger | 高 | DNS 传输未接 probe/tile；ProbeResult、真实 tile proxy 等仍缺 |
| 发布链 | GitHub/CI/部署 | main `32cb36d` / 腾讯 current `3bbcbfa`；FIX-BATCH-012 artifact/vault/state/health/双回环已核验；FIX-BATCH-013 模块未进可达 bundle | vault deployed / DNS transport main-not-deployed | 高 | 公网仍禁用；真实 Ovi 源与用户 accepted 未完成 |
...
- 19:08 `/System/Volumes/Data` 可用约 7.15 GiB，低于 8 GiB；swap 约 5.70 GiB 使用。全部本机构建、测试、浏览器、下载、影像和新制品部署已停止。
- 一条 `df && npm run build && npm run test:production` 组合命令只把 `df` 当作成功的只读命令，没有把 8 GiB 阈值编码为退出条件，因此在输出低容量后仍启动了 build/smoke。其通过结果保留为程序输出，但不计作符合过程门的最终验证。
- 新增 `OMB-AUD-040`：root 的 test/typecheck/build/dev/production smoke/compat/E2E/fixture acquisition 都由 npm `pre*` 脚本自动调用 `env:check`，静态 Node 测试冻结这组映射。最终分支在低容量下不做本地重型复验，交由 GitHub CI；这不影响此前 vault 核心的 local-verified 事实，也不把分支整体提升为 local-verified。
- PR #23 第一次 CI `33386547512` 由静态契约发现 fixture acquisition hook 拼写错误；第二次 `33386726268` 由单元正例发现 vault 命中仍被阻塞。修正后第三次 `33386874683` 和 squash main `3d1ddf2` 的 CI `33387017311` 均全绿，FIX-BATCH-012 与 OMB-AUD-040 达到 main。
- 19:30 本机仍约 7.14 GiB，因此没有为 main 重新本地构建，也没有部署新腾讯 release。此处阶段为 `code main / Tencent prior runtime deployed / real-source accepted false`。
