...
| Git 仓库 | GitHub main `f84ae20`；当前证据分支 `codex/audit-p1-temporal-input-truth-evidence` 基于该提交，remote 为 `https://github.com/LegalInvest/open-map-bridge.git` | `git branch --show-current`、`git rev-parse HEAD origin/main`、`git remote -v`、PR #9 | 2026-08-28 18:51 |
...
| 审计问题账本 | 161 个 tracked files 静态审计形成 38 组问题与公开 backlog；前五批已进 main，第五批统一时序输入真值并保留首次 CI 红灯 | FIX-BATCH-001/002/003/004/005 main / remaining discovered | `docs/问题账本.md`、PR #1/#3/#5/#7/#9、CI `33164783702`；2026-08-28 18:51 |
| 工作区基线 | GitHub main `f84ae20`；当前证据分支只回写 FIX-BATCH-005 的 main/CI 事实；约 5.2 GiB 低于 8 GiB 门 | product main / evidence candidate / capacity blocked | `git status/log/rev-parse`、`df`；2026-08-28 18:51 |
...
| GitHub main | 公共唯一主仓 `LegalInvest/open-map-bridge`；FIX-BATCH-005 产品由 PR #9 合并为 `f84ae20` | main for 001A + FIX-BATCH-001/002/003/004/005；证据回写在独立分支 | PR #1–#9、CI `33164783702`；2026-08-28 18:51 |
