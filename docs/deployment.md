...
FIX-BATCH-012 credential-vault and the OMB-AUD-040 automatic preflight guard reached main `3d1ddf2` after PR CI `33386874683` and main CI `33387017311` passed. The current Tencent `94e42b1` release does not contain it and no vault master key has been installed because local capacity is about 7.14 GiB, below the approved build/deploy gate. Enable the vault only after capacity recovers, the exact main production artifact passes the approved gates, and the versioned release is switched with the same state/health/loopback gates.
