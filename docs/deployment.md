...
> Current decision (2026-09-01): cloud deployment is deferred. The active delivery target is GitHub `main` plus the zero-context handoff package. The Tencent facts below are historical evidence and rollback context only; they do not authorize a new release, restart, tunnel, or server write.
...
FIX-BATCH-015 was squash merged by PR #41 as main `67a6a0e`; PR CI `33447491953`/`33447704065` and main CI `33447839812` passed the full gates. Exact main passed local production build/smoke and was copied to immutable release `/opt/open-map-bridge/releases/67a6a0e`; gateway/Web index SHA-256 `73808e31d2154aec2a9d0ffc8e270acccf04c5bd3b252054b8fcfc5f26f4eb8d`/`3dd8d87d7b2beb0f29b516ff2737807a86f12de064388a69a5d23b55e741d799` matched before `current` switched atomically from `d350ac3`. The first nginx health request returned 502, then bounded retry succeeded; service, health, 401, loopback listeners, state/vault hashes and 0600 passed. A tunneled browser showed all four synthetic panes as complete 6/6 with no console error/warning. This deploys honest frame-quality UI only; it does not persist ComparisonReceipt or prove any real source rendered/accepted.

FIX-BATCH-016 was squash merged by PR #44 as runtime main `3cf763a`; PR CI `33450167879`/`33450368931` and main CI `33450486867` passed the full gates. An exact local main build produced gateway/Web index SHA-256 `fcc835ecac27894d95d885b3b45f6ed13a38f71e3c33601e438ff2afd46d0762`/`9f2aa734c246a27f6360d6cd1727108645aded9dc2e9e7977f4e86d47423ee90`, both matching its manifest. This artifact has not been installed: the 2026-09-01 07:49 execution sandbox rejected Tencent SSH before any remote read or write. The last verified current remains `67a6a0e`; do not treat it as a live refresh. `DEC-20260901-001` now deliberately defers cloud deployment: do not install, restart, or switch Tencent releases until the user explicitly reauthorizes deployment after the future merge decision. If that authorization is later restored, install an explicitly selected immutable release and run the standard artifact/current/service/health/401/loopback/state/vault checks plus a ComparisonReceipt write-and-refresh visit before advancing to deployed.
