...
- `last_updated_at`: `2026-09-01T11:07:39.455+08:00`
- `basis_commit`: `1796a4b07b7a`
- `source_fingerprint_sha256`: `cdc4a1b8730da60f2f615197e3498509b5935325f7d608cd17c6d2932b34033e`
- `status`: `持续更新中的技术事实稿`
...
- `metadata`：所有已确认源可具有；
- `map-tiles`：普通非时序源只有在当前请求计划/凭据修订与 success ProbeResult 的脱敏绑定完全一致时可具有；
...
`FIX-BATCH-014C` 在阶段 B 之上形成显式、单坐标、不可充当开放代理的通用最小 probe。网关路由只接受已确认 source UUID、用户授权布尔值和严格 z/x/y，不接受客户端 URL、host、header 或任意 query；服务端重新读取同 UUID source，并要求静态 HTTPS 请求计划、字段 provenance、公开 query 和凭证需求都已被证明。vault 仍以同 UUID 解引用，新增 keyed HMAC-SHA-256 凭证修订指纹：凭证明文只在内存中用于构造受限 query/header，请求/结果指纹、状态、API、日志与回执均不含值。每次真正请求都重新执行 FIX-BATCH-013 DNS 全结果/IP/metadata/私网门，固定单地址 lookup 并核对 peer；默认生产依赖没有私网例外，HTTP、未知凭证需求和残余模板变量均在 DNS 前失败关闭。响应限制 5 MiB，只接受 PNG/JPEG 且必须通过完整解码/尺寸门；成功和稳定失败类别都原子写入脱敏 ProbeResult，同一指纹在并发与进程重启后复用，只有成功才把持久 source 从 confirmed 晋级 probed。该状态不自动注册 tiles/temporal-catalog runtime，也不等于 rendered/accepted。

03:57 的本地证据只使用进程内回环合成上游，并通过测试专用精确地址例外证明真实 pinned socket、路径变量、公开 query 和 vault query/header 均到达预期端点；生产默认策略不继承该例外。53 项定向测试、256 Vitest＋3 Node、8 workspace typecheck、production build/smoke 与 4 Chrome E2E 全绿；DNS 授权前 `externalRequest=false`、403、损坏/错误内容、策略拒绝、重复请求和重开均有反例。没有读取用户二维码载荷、真实凭证或真实第三方 host，也没有产生真实 Ovi/通用 ProbeResult。阶段为 `local-verified / not main / not deployed / not accepted`；main/腾讯仍分别是 `601ac68`/`ccd3cd8`。

PR #35 首轮 CI `33434618444` 对功能提交 `d3118b8` 完整通过，014C 晋级 `PR CI verified / not main / not deployed / not accepted`。该证据不改变 GitHub main `601ac68` 或腾讯 current `ccd3cd8`，也不产生真实 ProbeResult。

证据提交 CI `33434979533` 全绿后，PR #35 squash 合并为 main `7da03c3`，main CI `33435153343` 再次通过交底、256 Vitest＋3 Node、8 workspace typecheck、production build/smoke 与 4 Chrome。精确 main 本地重建和冒烟后，gateway/Web SHA-256 `a3d69a0e…56a1d`/`42368f3e…04ad` 与 manifest 和腾讯新 immutable release `7da03c3` 一致；current 从 `ccd3cd8` 原子切换。重启窗口首次 nginx health 返回 502，有限重试后 health 200，直连 401、4174/8080 双回环、service active，state/vault hash 与 0600 权限均未变化，旧 release 保留。该证据把 014C 推进到 `deployed-code`，没有真实图源、真实凭证或真实 ProbeResult；通用 tile/temporal runtime、rendered、accepted 仍未达到。

部署证据 PR #36 CI `33436046174` 全绿并合并为 docs-only main `84f103f`，main CI `33436199094` 再次通过完整门。本地/main/origin clean；腾讯 runtime/current 正确保留 `7da03c3`，health/401/双回环/artifact/state/vault 只读复核无漂移。该文档后代不生成第二个 runtime release，真实源阶段不变。

`FIX-BATCH-014D` 把普通非时序源的成功 probe 与最小瓦片运行时连接，但不伪造日期目录。原子状态只保存 source UUID、当前公开请求计划＋凭据 HMAC 修订的 SHA-256、成功 ProbeResult 指纹和时间；不保存 URL、host、凭证明文或响应正文。直接/V1 路由只接受 sourceId 与严格 z/x/y，拒绝任意 query；每次瓦片请求重新解析当前 source/vault、比对绑定，并复用 014C 请求构造、FIX-BATCH-013 DNS/IP/peer 固定传输、5 MiB、MIME 和完整图片解码门。凭据轮换或请求计划变化会立即撤销 `map-tiles`，重新成功 probe 后才恢复。3 Node＋257 Vitest、8 workspace typecheck、production build/smoke 和 4 Chrome E2E 在本机回环合成上游上通过。PR #38 两轮 CI 与 main CI 全绿，squash main/腾讯 current 为 `d350ac3`；gateway/Web hash、health/401/双回环/state/vault/0600 门通过。当前为 `deployed-code`，但仍未 real-probed/rendered/accepted，且未授予 `temporal-catalog/tiles`。

`FIX-BATCH-015` 针对多瓦片地图的画面质量假绿增加唯一瓦片状态机。每个挂载帧以 tile key 去重，分别累计 expected、loaded、failed；在任一已知瓦片仍 pending 时保持 loading，全部结算且全成功才是完整加载，成功与失败并存为 partial，全部失败为 failed。失败瓦片重试成功可替换旧结果，卸载时注销监听并停止旧回调。四屏界面公开显示三项计数。该批本身只证明前端质量判断逻辑，不等于真实画布 rendered；其当时尚缺的持久 ComparisonReceipt 由后续 FIX-BATCH-016 候选承接。

`FIX-BATCH-016` 把上述浏览器终态转成可持久、可回访且失败关闭的 ComparisonReceipt V1。共享 schema 只接受 4 个互异 dateId 和同序 4 帧；非 missing 帧必须 expected>0、loaded+failed=expected，并分别满足全成功、混合或全失败，missing 三计数必须为 0，loading/waiting 不可入库。POST 不接受客户端 ID/时间，服务端核对 ready source、精确 confirmed AOI 版本、当前 20 年日期目录及 missing 元数据后生成身份与 UTC 时间；repository 写入和重开复用同一 schema，并拒绝悬空 AOI。Web 仅在共享视角、四个互异日期和四帧终态齐备时启用保存，日期或 AOI 版本变化会撤销旧状态，刷新后按 source/AOI/version 显示历史条数。变更前腾讯权威 state 的 comparison 数为 0，因此没有猜测旧格式迁移。3 Node＋271 Vitest、8 workspace typecheck、production build/smoke、隔离 4 Chrome，PR #44 CI `33450167879`/`33450368931` 与 main CI `33450486867` 全绿；squash main 为 `3cf763a`。本轮精确 main build 与 manifest/hash 一致，gateway/Web SHA-256=`fcc835ec…d0762`/`9f2aa734…ee90`；由于执行沙箱阻止 SSH，当前是 main 而不是 deployed。合成回执不等于真实画布 rendered 或用户 accepted，Observation 保存仍独立开放。

部署证据 PR #39 CI `33440530796` 合并为 docs-only main `c4f0dcf`，main CI `33440688435` 全绿；运行源码和腾讯 current 保持 `d350ac3`，不为文档后代重复发布。
...
- 仅本机候选验证的普通源 `map-tiles` 脱敏绑定、凭据轮换撤销和逐请求固定传输；
- 跨平台 PNG/JPEG 像素归一化探针；
- GitHub `main`、Apache-2.0、公共 CI 和 Chrome E2E。

### 尚未完成，不得跨级声称

- 真实奥维二维码的私有认证桥接与第一条真实 ProbeResult；通用 vault 和 URL/IP SSRF 门虽已部署代码，但尚未用于真实源；
- 同一 imported source ID 到 OviBridge/标准适配器的真实探测和画布 rendered；014D map-tiles 仍未 main/deployed，且没有真实能力证据；
...
| 2026-09-01T11:07:39.455+08:00 | `1796a4b07b7a` | `cdc4a1b8730d` | 修正完整 Agent 交接的最终源码指纹 |
| 2026-09-01T11:05:59.786+08:00 | `56868dab6b3b` | `eb63b7efe5ce` | 补充真实遥感主线与跨项目 Agent 完整交接 |
| 2026-09-01T10:18:25.254+08:00 | `efb8305a93ac` | `45aa3fc2cd96` | 回写 GitHub 交接 main/CI 回执并冻结云端自动续发 |
| 2026-09-01T10:14:07.761+08:00 | `da34098649c0` | `3e13b966d947` | 建立 GitHub-only 零上下文交接、跨项目合并矩阵与云部署延期边界 |
| 2026-09-01T09:49:03.089+08:00 | `85ab0d3f3625` | `49fee4c1709d` | 记录09:48活动runner自行结束但容量门仍未解除 |
| 2026-09-01T09:47:57.878+08:00 | `f713ab3788e0` | `1c5fdad01503` | 记录09:46临界容量与跨项目活动runner下载阻塞 |
| 2026-09-01T08:48:11.359+08:00 | `31a1047a3fda` | `036d32a7b225` | 记录08:46容量门与网络执行阻塞，FIX-BATCH-016保持main未部署 |
| 2026-09-01T07:55:24.473+08:00 | `d150ac5768d3` | `5aed248f3c60` | synchronize FIX-BATCH-016 deployment and automation docs |
| 2026-09-01T07:53:22.971+08:00 | `3cf763a8ae08` | `044793e9c6d6` | refresh FIX-BATCH-016 main evidence tables |
| 2026-09-01T07:53:01.809+08:00 | `3cf763a8ae08` | `325fdcc771d6` | record FIX-BATCH-016 main and deployment sandbox boundary |
| 2026-09-01T07:21:49.100+08:00 | `5d3adac63987` | `27f07d48f404` | record FIX-BATCH-016 PR 44 verified CI |
| 2026-09-01T07:18:34.353+08:00 | `7f3854365d85` | `728f44baae98` | FIX-BATCH-016 final local verification |
| 2026-09-01T07:16:27.356+08:00 | `7f3854365d85` | `88d5fb3f9a1c` | FIX-BATCH-016 strict comparison receipt local-verified |
| 2026-09-01T06:57:57.678+08:00 | `f506a127f7c9` | `02775404ec52` | finalize FIX-BATCH-015 three-end evidence |
| 2026-09-01T06:53:03.547+08:00 | `67a6a0e092f8` | `413550023650` | record FIX-BATCH-015 main CI and Tencent release |
| 2026-09-01T06:45:43.503+08:00 | `b614845f8cb8` | `fa772daaf9e0` | record FIX-BATCH-015 PR 41 verified CI |
| 2026-09-01T06:42:48.929+08:00 | `96645f63e4b8` | `733cd569d515` | resume FIX-BATCH-015 after capacity recovery |
| 2026-09-01T05:59:51.271+08:00 | `c6cd5be0de84` | `63410d4f0482` | checkpoint FIX-BATCH-015 after capacity gate |
| 2026-09-01T05:58:20.131+08:00 | `c6cd5be0de84` | `5a8adb5ecdfe` | FIX-BATCH-015 local verified after E2E gate |
| 2026-09-01T05:56:38.318+08:00 | `c6cd5be0de84` | `c8b0b64072a5` | record FIX-BATCH-015 third E2E red gate and unify assertions |
| 2026-09-01T05:55:03.470+08:00 | `c6cd5be0de84` | `3ea4b82b9878` | record FIX-BATCH-015 second E2E red gate |
| 2026-09-01T05:53:27.958+08:00 | `c6cd5be0de84` | `f47c7659d0ea` | record FIX-BATCH-015 first E2E red gate |
| 2026-09-01T05:51:16.634+08:00 | `c6cd5be0de84` | `e87635f37eab` | FIX-BATCH-015 frame quality local candidate |
| 2026-09-01T05:22:15.781+08:00 | `c4f0dcfe5527` | `32daae9c6736` | --message finalize FIX-BATCH-014D three-end evidence |
| 2026-09-01T05:16:36.637+08:00 | `d350ac33c773` | `d877b8f4ab82` | --message record FIX-BATCH-014D main CI and Tencent release d350ac3 |
| 2026-09-01T05:05:11.362+08:00 | `72a054b1e5fb` | `0aeca30e5710` | --message record FIX-BATCH-014D PR 38 verified CI |
| 2026-09-01T05:01:17.542+08:00 | `a98d096ff23c` | `371a6d5f9d81` | --message FIX-BATCH-014D local-verified with server-side map tile permission |
| 2026-09-01T04:59:42.008+08:00 | `a98d096ff23c` | `ddddcbc6c1e2` | --message FIX-BATCH-014D generic map tile runtime local-verified |
| 2026-09-01T04:32:47.379+08:00 | `84f103f1a9da` | `b9ee3ca96616` | --message finalize FIX-BATCH-014C three-end evidence |
| 2026-09-01T04:26:08.945+08:00 | `7da03c39f904` | `7013615058f5` | --message record FIX-BATCH-014C main CI and Tencent release 7da03c3 |
| 2026-09-01T04:14:41.826+08:00 | `d3118b88838a` | `d4b5e64e5050` | --message record FIX-BATCH-014C PR 35 verified CI |
| 2026-09-01T04:09:48.411+08:00 | `d6f74982535e` | `a0e9af190206` | --message FIX-BATCH-014C generic probe local-verified |
| 2026-09-01T04:05:22.150+08:00 | `d6f74982535e` | `526cf989552d` | --message FIX-BATCH-014C generic probe local-verified |
