...
- `last_updated_at`: `2026-09-01T05:05:11.362+08:00`
- `basis_commit`: `72a054b1e5fb`
- `source_fingerprint_sha256`: `0aeca30e5710e98d2665ef1798f29b6b57f8be3fc192fb465d886fb42ff3bcae`
- `status`: `持续更新中的技术事实稿`
...
`FIX-BATCH-014D` 把普通非时序源的成功 probe 与最小瓦片运行时连接，但不伪造日期目录。原子状态只保存 source UUID、当前公开请求计划＋凭据 HMAC 修订的 SHA-256、成功 ProbeResult 指纹和时间；不保存 URL、host、凭证明文或响应正文。直接/V1 路由只接受 sourceId 与严格 z/x/y，拒绝任意 query；每次瓦片请求重新解析当前 source/vault、比对绑定，并复用 014C 请求构造、FIX-BATCH-013 DNS/IP/peer 固定传输、5 MiB、MIME 和完整图片解码门。凭据轮换或请求计划变化会立即撤销 `map-tiles`，重新成功 probe 后才恢复。3 Node＋257 Vitest、8 workspace typecheck、production build/smoke 和 4 Chrome E2E 在本机回环合成上游上通过。当前仅 `local-verified`；未 main/deployed/real-probed/rendered/accepted，且未授予 `temporal-catalog/tiles`。
