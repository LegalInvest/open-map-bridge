...
- `last_updated_at`: `2026-09-01T06:57:57.678+08:00`
- `basis_commit`: `f506a127f7c9`
- `source_fingerprint_sha256`: `02775404ec520eee8af7060b9201b661b5c698be426fa319ae42f30cf5f64c94`
- `status`: `持续更新中的技术事实稿`
...
`FIX-BATCH-015` 针对多瓦片地图的画面质量假绿增加唯一瓦片状态机。每个挂载帧以 tile key 去重，分别累计 expected、loaded、failed；在任一已知瓦片仍 pending 时保持 loading，全部结算且全成功才是完整加载，成功与失败并存为 partial，全部失败为 failed。失败瓦片重试成功可替换旧结果，卸载时注销监听并停止旧回调。四屏界面公开显示三项计数。该候选只证明前端质量判断逻辑，不等于真实画布 rendered；持久 ComparisonReceipt 仍待后续 API/UI 闭环。
