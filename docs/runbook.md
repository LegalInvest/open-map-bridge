...
Web 操作：完成授权保存后点击“检查图源准备度”，或从顶部进入“任务驾驶舱”选择已保存图源。逐步查看状态、是否外联和下一动作。旧版/未证请求字段、未知传输协议和明文 HTTP 默认进入人工门；私网/裸 IP/企业域名/非标准端口默认停在企业主机人工门；云元数据地址永久阻断；疑似凭证被脱敏而未配置、vault 未启用或引用不存在时停在凭证保险库门；没有运行时适配器时保持 blocked。凭证表单成功只证明加密存储和引用存在，不证明请求注入、服务器授权或瓦片有效。
