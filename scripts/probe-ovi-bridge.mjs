...
function decodeAndNormalize(bytes, format) {
  ...
  const normalized = Buffer.from(decoded.data);
  if (
    ...
    throw new Error('image decoder returned invalid RGBA pixels');
  }
...
for (const requestedDate of requestedDates) {
  ...
  const decoded = decodeAndNormalize(body, header.format);
